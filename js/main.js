...
(() => {
  ...
  /* ---------- 発展段階 ---------- */
  /* ---------- 静的マスターデータ（js/data.js から） ---------- */
  const D = window.LizaData;
  const STAGES = D.STAGES;
  const OPENING_LINES = D.OPENING_LINES;
  const FIRST_GOAL_TOAST = D.FIRST_GOAL_TOAST;
  const STORY = D.STORY;
  const STORYBOOK = D.STORYBOOK;
  ...
  const FIRST_BUILD_LINE = D.FIRST_BUILD_LINE;
  const FIRST_RESIDENT_LINE = D.FIRST_RESIDENT_LINE;

  /* 図鑑キャラページの立ち絵（js/data.js から） */
  const FULL_PORTRAITS = D.FULL_PORTRAITS;
  ...
  /* ---------- 住人の節目（職業＋一言。js/data.js から） ---------- */
  const RESIDENT_MILESTONES = D.RESIDENT_MILESTONES;
  ...
  /* 祝祭演出：おめでとう!＋光の粒＋住人たちの歓声（CHEERS は js/data.js から） */
